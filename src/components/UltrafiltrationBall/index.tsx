...
const UltrafiltrationBall: React.FC<UltrafiltrationBallProps> = ({
  ...
  const drawWaves = useCallback((
    ...
    const wave = (x: number, wavelength: number, amplitude: number, offset: number) =>
    ...
    const drawWave = (
      ...
      // 调整动态透明度的范围，使波浪更加明显
      const dynamicAlpha = baseAlpha + 0.15 * Math.sin(offset * 0.002 + phaseShift);
  ...
  const drawBall = useCallback((
    ...
    for (let i = 0; i < totalSegments; i += segmentBatch) {
      ...
      for (let j = 0; j < segmentBatch && (i + j) < totalSegments; j++) {
        const startAngle = -Math.PI / 2 + (fillAngle / totalSegments) * (i + j);
        const endAngle = -Math.PI / 2 + (fillAngle / totalSegments) * (i + j + 1);
    ...
    if (canvasReady) {
      ...
      if (animate && !animationRef.current) {
        ...
        if (canvasRef.current) {
          const { ctx, width, height } = canvasRef.current;
          drawBall(ctx, width, height, value, Math.max(1, maxValue), performance.now());
        }
      }
    }
  }, [value, animate, canvasReady, animateBall, drawBall, maxValue, stopAnimation]);
